...
pub fn message_to_lsp_diagnostic(
    ...
    let spans = message.error.labels.as_slice();
    let related_information = (!spans.is_empty()).then(|| {
        spans
            .iter()
            .map(|span| {
                ...
                for rule in rules {
                    reports.push(build_unused_disable_diagnostic_report(
                        rule.directive_prefix
                            .unused_disable_rule_message(&rule.rule_name),
                        rule.name_span,
                        severity,
                        source_text,
                        rope,
...
fn add_ignore_fixes(
    ...
    debug_assert!(
        !fixes
            .iter()
            .any(|fix| fix.message.starts_with("remove unused disable directive")),
...
fn disable_for_this_line(
    ...
    for byte in bytes[section_offset as usize..error_offset as usize]
        .iter()
        .rev()
    {
        if *byte == b'\n' || *byte == b'\r' {
...
mod test {
    ...
    fn add_ignore_fixes_uses_user_facing_plugin_names() {
        ...
        let code = OxcCode {
            scope: Some("jsx-a11y".into()),
            number: Some("alt-text".into()),
        };
        let mut fixes = vec![];
